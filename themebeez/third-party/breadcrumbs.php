 ...
 */
function royale_news_breadcrumb_trail( $args = array() ) {

	$breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new Royale_News_Breadcrumb_Trail( $args );
	}
 ...
 * @access public
 */
class Royale_News_Breadcrumb_Trail {
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:', 'royale-news' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'royale-news' ),
			'home'                => esc_html__( 'Home', 'royale-news' ),
			'error_404'           => esc_html__( '404 Not Found', 'royale-news' ),
			'archives'            => esc_html__( 'Archives', 'royale-news' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s', 'royale-news' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'royale-news' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', 'royale-news' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'royale-news' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'royale-news' ),
	...
	protected function set_post_taxonomy() {
		...
		if ( '%postname%' === trim( get_option( 'permalink_structure' ), '/' ) ) // phpcs:ignore
	...
	protected function add_items() {
		...
		if ( is_front_page() ) { // If viewing the front page.
	...
	protected function add_front_page_items() {
		...
		if (
			...
			if ( is_paged() ) { // If on a paged view, add the site home link.
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'royale-news' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'royale-news' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'royale-news' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'royale-news' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'royale-news' ) ) );
		$day   = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'royale-news' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'royale-news' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'royale-news' ) ) );
	...
	protected function add_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			if (
				'page' == $post->post_type && // phpcs:ignore
				'page' == get_option( 'show_on_front' ) && // phpcs:ignore
				$post_id == get_option( 'page_on_front' ) // phpcs:ignore
			) {
				break;
			}
	...
	protected function add_post_hierarchy( $post_id ) {
		...
		if (
			...
			false !== strpos( $post_type_object->rewrite['slug'], '%' )
		) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) { // phpcs:ignore
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'royale-news' ) ) ) );

					// If using the %monthnum% tag, add a link to the monthly archive.
				} elseif ( '%monthnum%' == $tag ) { // phpcs:ignore
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'royale-news' ) ) ) );

					// If using the %day% tag, add a link to the daily archive.
				} elseif ( '%day%' == $tag ) { // phpcs:ignore
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'royale-news' ) ) ) );
